...
function handleMouseClick(e) {
  ...
  if (e.target.matches("[data-key]")) {
    ...
    if (/[a-zA-Z]/i.test(charStr)) {
      const empty = document.getElementsByClassName(
        `${emptyCell} Row${emptyRow}`
      )[0];
      if (empty) {
        empty.innerText = e.target.textContent.toUpperCase();
        emptyCell++;
      }
    }
    return;
  ...
  if (e.target.matches("[data-enter]")) {
    if (emptyCell > 5 && !gameFinished) {
      for (let i = 1; i < 6; i++) {
        wordArray.push(
          document
            .getElementsByClassName(`${i} Row${emptyRow}`)[0]
            .textContent.toLowerCase()
        );
        word += document.getElementsByClassName(`${i} Row${emptyRow}`)[0]
          .textContent;
      }
...
function handleKeyDown(e) {
  ...
  if (/[a-zA-Z]/i.test(charStr)) {
    ...
    if (empty) {
      empty.innerText = e.key.toUpperCase();
      emptyCell++;
    }
  }
  if (charCode == 13 && emptyCell > 5 && !gameFinished) {
    for (let i = 1; i < 6; i++) {
      wordArray.push(
        document
          .getElementsByClassName(`${i} Row${emptyRow}`)[0]
  ...
  for (let i = 0; i < 5; i++) {
    if (correctWordArray[i] == wordArray[i]) {
      ...
      document
        ...
        .classList.add("correct");
      countLetter[wordArray[i].charCodeAt(0) - 97]--;
    } else {
      flag = 1;
    }
  }
